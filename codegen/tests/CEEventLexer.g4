...
EVENT_AB:     'ab'     -> pushMode(MODE_BITS);   // Array: Bits
...
EVENT_ADU:    'adu'    ;                         // Array Data: UID
EVENT_AF16:   'af16'   -> pushMode(MODE_FLOAT);  // Array: Float16
EVENT_AF32:   'af32'   -> pushMode(MODE_FLOAT);  // Array: Float32
EVENT_AF64:   'af64'   -> pushMode(MODE_FLOAT);  // Array: Float64
EVENT_AI16:   'ai16'   -> pushMode(MODE_INT);    // Array: Int16
EVENT_AI32:   'ai32'   -> pushMode(MODE_INT);    // Array: Int32
EVENT_AI64:   'ai64'   -> pushMode(MODE_INT);    // Array: Int64
EVENT_AI8:    'ai8'    -> pushMode(MODE_INT);    // Array: Int8
EVENT_AU16:   'au16'   -> pushMode(MODE_UINT);   // Array: Uint16
EVENT_AU16X:  'au16x'  -> pushMode(MODE_UINTX);  // Array: Uint16 (hex)
EVENT_AU32:   'au32'   -> pushMode(MODE_UINT);   // Array: Uint32
EVENT_AU32X:  'au32x'  -> pushMode(MODE_UINTX);  // Array: Uint32 (hex)
EVENT_AU64:   'au64'   -> pushMode(MODE_UINT);   // Array: Uint64
EVENT_AU64X:  'au64x'  -> pushMode(MODE_UINTX);  // Array: Uint64 (hex)
EVENT_AU8:    'au8'    -> pushMode(MODE_UINT);   // Array: Uint8
EVENT_AU8X:   'au8x'   -> pushMode(MODE_UINTX);  // Array: Uint8 (hex)
...
EVENT_CB:     'cb'     -> pushMode(MODE_UINTX);  // Custom Binary
EVENT_CM:     'cm' F_CHAR_NONNUL+;               // Comment: Multiline
EVENT_CS:     'cs' F_CHAR_NONNUL+;               // Comment: Single Line
EVENT_CT:     'ct'     -> pushMode(MODE_STRING); // Custom Text
...
EVENT_REFR:   'refr'   -> pushMode(MODE_STRING); // Reference: Remote
EVENT_RID:    'rid'    -> pushMode(MODE_STRING); // Resource ID
EVENT_SI:     'si'     -> pushMode(MODE_STRING); // Struct Instance
EVENT_ST:     'st'     -> pushMode(MODE_STRING); // Struct Template
EVENT_S:      's'      -> pushMode(MODE_STRING); // String
